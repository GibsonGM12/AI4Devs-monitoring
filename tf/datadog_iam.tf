# ------------------------------------------------------------------------------
# IAM para integración AWS-Datadog
# Role que Datadog asume para leer métricas/logs de AWS. Trust policy usa el
# external_id generado por datadog_integration_aws_account.
# ------------------------------------------------------------------------------
...
data "aws_iam_policy_document" "datadog_aws_integration_assume_role" {
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${var.datadog_aws_principal_account_id}:root"]
    }
    condition {
      test     = "StringEquals"
      variable = "sts:ExternalId"
      values   = [datadog_integration_aws_account.datadog_integration.auth_config.aws_auth_config_role.external_id]
    }
  }
}
